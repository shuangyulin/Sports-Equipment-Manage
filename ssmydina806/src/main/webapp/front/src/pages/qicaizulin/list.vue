	...
	<div>
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-button class="list-add-btn" v-if="btnAuth('qicaizulin','新增')" type="primary" @click="add('/index/qicaizulinAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)">
						...
						<div class="infoBox">
							<div class="name">器材编号:{{item.qicaibianhao}}</div>
							<div class="name">{{item.qicaileixing}}</div>
							<div class="name">{{item.zulinriqi}}</div>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`qicaizulin/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/qicaizulinDetail', query: params});
			},
