<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="器材编号" prop="qicaibianhao">
				<el-input v-model="ruleForm.qicaibianhao" 
					placeholder="器材编号" clearable :disabled=" false  ||ro.qicaibianhao"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="器材名称" prop="qicaimingcheng">
				<el-input v-model="ruleForm.qicaimingcheng" 
					placeholder="器材名称" clearable :disabled=" false  ||ro.qicaimingcheng"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="器材类型" prop="qicaileixing">
			...
			<el-form-item class="add-item" label="租赁日期" prop="zulinriqi">
				<el-date-picker
					:disabled=" false  ||ro.zulinriqi"
			...
			<el-form-item class="add-item" label="租赁数量" prop="shuliang">
				<el-input v-model.number="ruleForm.shuliang" 
					placeholder="租赁数量" clearable :disabled=" false  ||ro.shuliang"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="租赁备注" prop="zulinbeizhu">
				<editor 
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.zulinzhuangtaiOptions = "已归还,已租赁".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`qicaizulin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(table+`/update`,obj).then(res => {});
						await this.$http.post(`qicaizulin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
