	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/qicaizulin?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">器材名称</div>
						<div class="text "  >{{detail.qicaimingcheng}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">租赁日期</div>
						<div class="text "  >{{detail.zulinriqi}}</div>
					</div>
					<div class="item">
						<div class="lable">租赁数量</div>
					...
					<div class="item">
						<div class="lable">租赁状态</div>
						<div class="text "  >{{detail.zulinzhuangtai}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('qicaizulin','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('qicaizulin','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('qicaizulin','器材归还')" 
							@click="onAcross('qicaiguihai','','','zulinzhuangtai','已归还','已归还,已租赁'.split(',')[0])" type="warning">
							器材归还
						</el-button>
					</div>
				</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="租赁备注" name="first">
					<div class="ql-snow ql-editor" v-html="detail.zulinbeizhu"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.qicaibianhao;
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`qicaizulin`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/qicaizulin', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/qicaizulinAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此器材租赁？') .then(_ => {
					this.$http.post('qicaizulin/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
